...
def add_publish_permission(apps, schema_editor):
    ...
    users = User.objects.filter(Q(user_permissions__codename='change_content', user_permissions__content_type__app_label='cfblog'))
    
    Group = apps.get_model('auth', 'Group')
    groups = Group.objects.filter(Q(permissions__codename='change_content', 
